...
export async function parseMongoConnectionString(connectionString: string): Promise<ParsedMongoConnectionString> {
    ...
    const clientOptions: MongoOptions = mongoClient.options;
    const hosts = clientOptions?.hosts;
    if (!!hosts && hosts.length > 0) {
        const host: string | undefined = hosts[0].host;
        const port: string | undefined = hosts[0].port?.toString();
        if (host !== undefined && port !== undefined) {
            ...
            throw new Error("Host missing hostname or port in connection string");
        }
    } else {
        throw new Error("No host in connection string");
    }
}
